...
//                //소셜인증서버가 발급한 인가코드로 소셜인증서버에게 access 토큰을 발급 받을때 refresh 토큰을 함께 발급해서 access 토큰 만료시 재로그인 할 필요가 없게 한다는 의미
//                //쇼셜인증서버가 발급해준 access 토큰은 사용자의 최초 로그인 이후에도 소셜서버에게 사용자 정보를 받아와야하는 요청을 사용자가 백앤드에게 했을 때 필요함
//                //백앤드는 소셜인증서버가 발급해준 access/refresh token을 사용자 별로 저장하고 관리하여야 함
//                .toUriString();
//
...
//            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("소셜 인증 서버 access token 취득 실패");
//        }
//
//
